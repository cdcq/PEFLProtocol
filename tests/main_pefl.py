...
def run_edge():
    ...
    model = get_model(model_name="mlp")
...
if __name__ == "__main__":
    ...
    # torch.multiprocessing.set_start_method("serverfork")

    register_users()
    kgc_process = Process(target=run_kgc, args=())
    kgc_process.start()
    sleep(3)
    cp_process = Process(target=run_cloud_provider, args=())
    cp_process.start()
    sleep(2)
    sp_process = Process(target=run_service_provider, args=())
    sp_process.start()
    sleep(2)
    ...
    for i in range(1):
        t_process = Process(target=run_edge, args=())
        t_process.start()
    ...
    # t_process = [0, 0, 0]
    # for i in range(1):
    #     t_process[i] = Process(target=run_edge, args=(), daemon=True)
    #     t_process[i].start()
    #
